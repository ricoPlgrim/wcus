        ...
        <div class="content-wrap display-wrap">
            <div class="detail-wrap lookbook"> <!-- lookbook 클래스 추가시 타입 B로 변환 -->
                ...
                <div class="detail-top on up" ref="detailTopRef">
                    <!-- 타이틀 컴포넌트 (위시리스트 클릭 이벤트 포함) -->
                    <TopTitle v-bind="topTitleProps" @wishClick="handleWishClick" />
                ...
                <div class="detail-scroll" ref="detailContents">
                    ...
                    <div ref="planComponentRef">
                        <!-- 상단 탭 영역 및 제품 리스트를 포함한 PlanComponent 컴포넌트 -->
                        <PlanComponent :products="newProducts" :options="options" @update-sticky-ref="updateStickyRef"
                            @update-btn-wrap-ref="updateBtnWrapRef" @open-popup="openPopup" />
                    </div>

                    <!-- 관련된 스토리 섹션 표시 -->
                    <RelatedStories :title="storiesTitle" :stories="relatedStories" />
                </div>
            </div>
        </div>
    </div>
...
import { useScroll } from '../../utils/useScroll';  // 스크롤 유틸리티 훅
...
import productHtml from './ProductHtml.vue'; // HTML 출력 컴포넌트
import RelatedStories from './RelatedStories.vue'; // 관련 스토리 컴포넌트
import PlanComponent from './PlanComponent.vue'; // 제품 목록을 포함하는 컴포넌트
import SelectPopupLayer from '../../components/Mo/Popup/SelectOptionPopup.vue'; // 옵션 선택 팝업 레이어
...
const handleShopClick = (data: { slide: Slide, index: number }) => {
    ...
    // Instead of passing item0/item1 directly, structure it properly
...
const onScrollDown = () => {
    if (detailTopRef.value) {
        if (detailTopRef.value.classList.contains('up')) {
            detailTopRef.value.classList.remove('up');
        }
        detailTopRef.value.classList.add('down');
    }
    if (quickProductRef.value) {

        if (quickProductRef.value.classList.contains('up')) {
            quickProductRef.value.classList.remove('up');
        }
        quickProductRef.value.classList.add('down');
    }
};
...
const onScrollUp = () => {
    if (detailTopRef.value) {
        if (detailTopRef.value.classList.contains('down')) {
            detailTopRef.value.classList.remove('down');
        }
        detailTopRef.value.classList.add('up');
    }
    if (quickProductRef.value) {
        if (quickProductRef.value.classList.contains('down')) {
            quickProductRef.value.classList.remove('down');
        }
        quickProductRef.value.classList.add('up');
    }
};
    ...
    nextTick(() => {
        ...
        if (targetElement && targetIndex !== null) {
            ...
            if (scrollTop < elementTop) {
                ...
                if (hasDepth01) {
                    scrollMove = elementTop; // depth-01이 있을 때 스크롤 위치
                } else {
                    scrollMove = elementTop - 38; // depth-01이 없을 때 스크롤 위치
                }
            } else {
                ...
                if (hasDepth01) {
                    ...
                    scrollMove = elementTop - 100; // depth-01이 없을 때 스크롤 위치
                }
            }
